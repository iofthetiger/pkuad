...
exp_lr_scheduler = lr_scheduler.OneCycleLR(optimizer, max_lr=0.005, steps_per_epoch=len(train_loader), epochs=n_epochs)
...
for epoch in range(n_epochs):
    ...
    gc.collect()
    train(epoch, history)
    torch.save(model.state_dict(), './b0_sz-2052-1026_onepolicy_ep-%s.pth'%(epoch))
    evaluate(epoch, history)
...
torch.save(model.state_dict(), './b0_sz-2052-1026_onepolicy_final.pth')
...
test.to_csv('b0_sz-2052-1026_onepolicy.csv', index=False)
test.head()
